...
<div class="px-4 sm:px-6 lg:px-8">
    <div class="sm:flex sm:items-center mb-8">
        <div class="sm:flex-auto">
            <h1 class="text-2xl font-bold text-gray-900">Table Booking</h1>
            <p class="mt-2 text-sm text-gray-600">Manage Booking for  Tables</p>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-150 cursor-pointer group" onclick="window.location='{{ route('admin.table.show', $table->id) }}'">
                        <td class="pl-6 pr-3 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="ml-4">
                                    <div class="text-sm font-semibold text-gray-900">{{ $table->name }}</div>
                                    <div class="text-xs text-gray-500 mt-1">{{ $table->created_at->format('M d, Y h:i A') }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-3 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">{{ $table->phone }}</div>
                        </td>
                        <td class="px-3 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                @if($table->is_read)
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center justify-center space-y-3">
                                ...
                                <p class="text-gray-400 text-sm">No Table Bookings found</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
            ...
            <div class="border-t border-gray-100 px-6 py-4">
                <nav class="flex items-center justify-between">
                    <div class="text-sm text-gray-500">
                        Showing {{ $tables->firstItem() }} - {{ $tables->lastItem() }} of {{ $tables->total() }}
                    </div>
                    <div class="flex space-x-2">
                        @foreach ($tables->links()->elements as $element)
                            @if(is_array($element))
